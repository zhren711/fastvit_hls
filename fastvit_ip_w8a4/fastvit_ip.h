...
/* W8A4 REDESIGN (2026-08-01, fastvit_ip_w8a4/, see project plan at
 * C:\Users\zhren\.claude\plans\wondrous-roaming-yeti.md): weights stay
 * 8-bit (wt_t unchanged), activations shrink from 8-bit to 4-bit
 * (act_t: ap_int<8> -> ap_int<4>). Symmetric signed range is -7..7 (NOT
 * -8..7) to match the Python-side quantization convention in
 * tools/export_weights_w8a4.py / calibrate_activations.py, which both
 * use 2**(nbits-1)-1 as the clip bound for both weights and activations
 * (consistent, not an arbitrary choice -- changing one side without the
 * other would silently break bit-exactness between the HLS csim
 * reference and the Python-computed quantized weights/scales).
 *
 * This one typedef change ripples through every worker's PRELOAD/
 * WRITEBACK/LOAD/STORE pack_t<->act_t bit-slicing: 4 lanes/32-bit-word
 * (8-bit slices) becomes 8 lanes/word (4-bit nibble slices) everywhere
 * act_t values cross the pack_t DRAM bus. wt_t-typed ports (in_b, used
 * natively unpacked by conv/dwconv/pwconv for weights) are UNCHANGED --
 * weights stay 8-bit, no repacking needed there.
 *
 * KNOWN GAP left unresolved in this pass (accuracy correctness is
 * explicitly deferred per user decision, see project notes): gelu_worker's
 * LUT is a static table indexed only by the 4-bit code, baked at
 * synthesis time to ONE assumed scale -- it has no way to apply the
 * actual per-layer CALIBRATED scale from Phase 1 (each GELU call site in
 * the real network sees a different real-valued scale). This mirrors a
 * pre-existing limitation in the original W8A8 design (which also
 * assumed one uniform 1/127 scale everywhere, matching the old
 * uncalibrated placeholder) -- it is not a new bug introduced here, just
 * now made explicit and worth fixing whenever accuracy work resumes. */
