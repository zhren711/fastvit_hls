 ...
 * jaunty-questing-journal.md). The tile-based design
 * (fastvit_ip_w8a4/dwconv_worker.cpp, UNCHANGED by this file) plateaued
 * at isolated WNS=-0.196ns after several rounds of targeted HLS fixes;
 * its final remaining critical path was a runtime-computed index
 * (`ic = c*stride_w+kw`) reading a fully-partitioned array
 * (`row_buf[ic]`) -- a ~13:1 dynamic-index mux. This rewrite eliminates
 * every dynamic-index read/write in the hot per-pixel path by replacing
 * the "DMA a whole tile into BRAM, then gather a window with computed
 * offsets" scheme with the classical streaming line-buffer/shift-window
 * architecture: a small set of Win-wide row buffers plus a KxK register
 * window, addressed only by fixed (compile-time-unrolled) indices and
 * the natural raster-scan column counter.
 *
