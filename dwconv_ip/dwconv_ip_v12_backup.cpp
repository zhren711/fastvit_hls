 ...
 * enables simultaneous access for unrolled COMPUTE loop */
...
#pragma HLS UNROLL
                    ...
                    /* Compute — PIPELINE on KW (innermost variable loop),
                     * UNROLL TR/TC (compile-time constants DW_TR=4, DW_TC=4).
                     * Reduces pipeline restarts from 196/tile to Kh=7/tile.
                     * Each KW cycle: DW_TR×DW_TC = 16 parallel MACs.
                     * Expected: Kh × (depth + Kw) ≈ 7×12 = 84 cycles/tile */
                    COMPUTE_DW_KH:
                    for (int kh = 0; kh < Kh; kh++) {
                        COMPUTE_DW_KW:
                        for (int kw = 0; kw < Kw; kw++) {
#pragma HLS PIPELINE II=1
                            ...
                            for (int r = 0; r < DW_TR; r++) {
#pragma HLS UNROLL
                                /* r is a compile-time constant after unroll;
                                 * stride_h is runtime → generates efficient shift/add */
                                int ir = r * stride_h + kh;
                                COMPUTE_DW_TC:
                                for (int c = 0; c < DW_TC; c++) {
#pragma HLS UNROLL
                                    int ic = c * stride_w + kw;
                                    act_t fi = dw_in_buf[0][ir][ic];
                                    wt_t  w  = dw_wt_buf[0][kh][kw];
                                    dw_out_buf[0][r][c] += (acc_t)fi * (acc_t)w;
                                }
                            }
                        }
                    }
