# run_impl_conv_only_200mhz.tcl -- conv-only Tier B minimal harness:
# Block Design + synth + impl at 200MHz, to test whether conv_worker
...
# Companion to package_conv_only_ip.tcl (must run first). See
# typed-knitting-nygaard.md plan for context: HLS's own csynth estimate
# for conv_worker already misses 5ns in isolation (pre-P&R); this is
# the authoritative post-P&R check.
#
# 用法: vivado -mode batch -source run_impl_conv_only_200mhz.tcl -nolog -nojournal

set_param general.maxThreads 1

set part      "xc7z020clg400-1"
set proj_name "fastvit_conv_only_200mhz_proj"
set script_dir [file normalize [file dirname [info script]]]
set proj_dir   "$script_dir/$proj_name"

set ip_dir "$script_dir/../fastvit_ip_w8a4/tier_b_single_ip/conv"
...
create_bd_cell -type ip -vlnv user.org:hls:conv_only_top:1.0 worker_0
...
foreach m {gmem0 gmem1 gmem2 gmem3} {
    ...
    connect_bd_intf_net [get_bd_intf_pins worker_0/m_axi_conv_$m] [get_bd_intf_pins sc_main/$si]
    incr idx
}
...
# dominated. Applying here since conv was never revisited after the
# original 2026-08-06 isolation experiment (-0.854ns).
...
open_run impl_1
set rpt_file "$proj_dir/utilization_conv_only_200mhz.rpt"
report_utilization -file $rpt_file

set timing_rpt "$proj_dir/timing_detail_conv_only_200mhz.rpt"
report_timing -delay_type max -max_paths 10 -sort_by group -file $timing_rpt
puts ">>> Detailed timing report: $timing_rpt"
puts ""
puts "========================================"
puts " POST-IMPLEMENTATION UTILIZATION (conv-only, 200MHz)"
puts "========================================"
report_utilization
