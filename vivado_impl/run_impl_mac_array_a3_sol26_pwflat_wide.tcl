# run_impl_mac_array_a3_sol26_pwflat_wide.tcl -- A3 run_layer rewrite
# stage 3 retry (ZHR-92, 2026-08-24): X0-96 failed to route (376
# unroutable pins), all tracing to ONE net -- gmem_meta's AXI read-FIFO
# output bus -- with Slice occupancy (84.5%) notably higher than raw LUT
# (57.69% real, confirmed post-placement). Same diagnostic signature as
# the DW-burst round's own precedent (design grew, pblock still sized
# for the smaller pre-growth design, one net's routing reach exceeded
# what the region could route, not a capacity overflow) -- fixed there
# by widening X0-96->X0-120, one shot. Doing the same here: this design
# also grew (PW's flat pipeline vs. the old nested-loop form) while the
# pblock stayed sized for the prior, smaller design -- exactly the
# CLAUDE.md rule's scenario, not a default carry-forward.
#
# One shot only -- if this also fails to route, STOP and report; do not
# try a third size in the same round (pblock size is a continuous
# parameter this project has falsified blind-search attempts on four
# separate times already).
#
# 用法: vivado -mode batch -source run_impl_mac_array_a3_sol26_pwflat_wide.tcl -nolog -nojournal
#
# Gotcha carried over from the X0-96 attempt: solution26's exported
# component.xml has vendor=xilinx.com, NOT user.org like solution22's did
# -- VLNV below already corrected.

set_param general.maxThreads 1

set part      "xc7z020clg400-1"
set proj_name "a3wp24"
