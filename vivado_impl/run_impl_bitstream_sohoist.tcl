# run_impl_bitstream_sohoist.tcl -- ZHR-92 (2026-09-12): real P&R for
# SCALAR_OP_SIZE_HOIST (hw/total computed once, unconditionally, in
# mac_array_top; passed as scalar params into the six scalar ops). Binding
# DB confirmed: zero op_type-predicated Multiplier ops left at top level,
# run_gelu/run_add 0 multiplier ops each (were 4), the shared 32x32
# multiplier's operand mux down to 2 arms. DW_OUTPUT_BURST OFF (CROW_CCOL
# II=8 = baseline). Same BD topology as the elemwise_burst baseline.
# Phase 1: route_design alone (phys_opt disabled), per the two-phase
# recipe. Judgment: WNS vs the baseline's own route_design-alone -0.167ns
# -- >=+0.2ns improvement re-opens DW_OUTPUT_BURST; ~0.05ns means the
# sink's bottleneck is not the operand side.
set_param general.maxThreads 1

set part      "xc7z020clg400-1"
set proj_name "sohoistimp"
set script_dir [file normalize [file dirname [info script]]]
set proj_dir   "$script_dir/$proj_name"

set ip_mac_array "$script_dir/../fastvit_ip_v2/sohoist_export/s1/impl/ip"
...
open_run impl_1
set rpt_file "$proj_dir/utilization_sohoist.rpt"
