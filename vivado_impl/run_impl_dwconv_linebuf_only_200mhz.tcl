# run_impl_dwconv_linebuf_only_200mhz.tcl -- isolated 200MHz P&R for the
# line-buffer/shift-register dwconv rewrite (dwconv_linebuf/dwconv_worker.cpp,
# see plan C:\Users\zhren\.claude\plans\jaunty-questing-journal.md). Directly
# uses the IP catalog output from `export_design -format ip_catalog`
# (dwconv_linebuf_proj/solution1/impl/ip) -- unlike the Tier B single-worker
# harnesses (dwconv_only_top.v etc.), this worker was synthesized standalone
# from the start (not extracted from a merged multi-worker design), so its
# HLS-exported top module already has clean, self-contained AXI4/AXI4-Lite
# ports and needs no hand-composed wrapper.
#
# Same placement/phys_opt directive recipe as every prior isolated-worker
# run in this investigation (run_impl_dwconv_only_200mhz.tcl et al.) for a
# fair, directly comparable WNS/LUT number.
#
# 用法: vivado -mode batch -source run_impl_dwconv_linebuf_only_200mhz.tcl -nolog -nojournal

set_param general.maxThreads 1

set part      "xc7z020clg400-1"
set proj_name "fastvit_dwconv_linebuf_only_200mhz_proj"
set script_dir [file normalize [file dirname [info script]]]
set proj_dir   "$script_dir/$proj_name"

set ip_dir "$script_dir/../fastvit_ip_w8a4/dwconv_linebuf/dwconv_linebuf_proj/solution1/impl/ip"
...
create_bd_cell -type ip -vlnv user.org:hls:dwconv_worker:1.0 worker_0
...
foreach m {gmem0 gmem1 gmem2 gmem3} {
    ...
    connect_bd_intf_net [get_bd_intf_pins worker_0/m_axi_$m] [get_bd_intf_pins sc_main/$si]
    incr idx
}
...
# Same placement/phys_opt recipe used for every prior isolated-worker
# 200MHz run in this investigation (dwconv/pwconv/conv/add-only harnesses),
# for a fair, directly comparable number.
...
open_run impl_1
set rpt_file "$proj_dir/utilization_dwconv_linebuf_only_200mhz.rpt"
report_utilization -file $rpt_file

set timing_rpt "$proj_dir/timing_detail_dwconv_linebuf_only_200mhz.rpt"
report_timing -delay_type max -max_paths 10 -sort_by group -file $timing_rpt
puts ">>> Detailed timing report: $timing_rpt"
puts ""
puts "========================================"
puts " POST-IMPLEMENTATION UTILIZATION (dwconv_linebuf-only, 200MHz)"
puts "========================================"
report_utilization
