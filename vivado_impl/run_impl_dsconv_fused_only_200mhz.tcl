# run_impl_dsconv_fused_only_200mhz.tcl -- isolated 200MHz P&R for the
# fused DW7+PW1 on-chip patch-level kernel (dsconv_fused/dsconv_worker.cpp,
# see plan C:\Users\zhren\.claude\plans\peppy-gathering-ripple.md). Directly
# uses the IP catalog output from `export_design -format ip_catalog`
# (dsconv_fused_proj/solution1/impl/ip) -- same pattern as
# run_impl_dwconv_linebuf_only_200mhz.tcl, this worker was synthesized
# standalone from the start, so its HLS-exported top module already has
# clean, self-contained AXI4/AXI4-Lite ports and needs no hand-composed
# wrapper.
#
# Same placement/phys_opt directive recipe as every prior isolated-worker
# run in this investigation, for a fair, directly comparable WNS/LUT
# number against dwconv_linebuf (-0.236ns) and pwconv_worker's own
# isolated number.
#
# 用法: vivado -mode batch -source run_impl_dsconv_fused_only_200mhz.tcl -nolog -nojournal

set_param general.maxThreads 1

set part      "xc7z020clg400-1"
set proj_name "fastvit_dsconv_fused_only_200mhz_proj"
set script_dir [file normalize [file dirname [info script]]]
set proj_dir   "$script_dir/$proj_name"

set ip_dir "$script_dir/../fastvit_ip_w8a4/dsconv_fused/dsconv_fused_proj/solution1/impl/ip"
...
create_bd_cell -type ip -vlnv user.org:hls:dsconv_worker:1.0 worker_0
...
set_property CONFIG.NUM_SI 5 [get_bd_cells sc_main]
...
set idx 0
foreach m {gmem0 gmem1 gmem2 gmem3 gmem4} {
...
# 200MHz run in this investigation, for a fair, directly comparable number.
...
open_run impl_1
set rpt_file "$proj_dir/utilization_dsconv_fused_only_200mhz.rpt"
report_utilization -file $rpt_file

set timing_rpt "$proj_dir/timing_detail_dsconv_fused_only_200mhz.rpt"
report_timing -delay_type max -max_paths 10 -sort_by group -file $timing_rpt
puts ">>> Detailed timing report: $timing_rpt"
puts ""
puts "========================================"
puts " POST-IMPLEMENTATION UTILIZATION (dsconv_fused-only, 200MHz)"
puts "========================================"
report_utilization
