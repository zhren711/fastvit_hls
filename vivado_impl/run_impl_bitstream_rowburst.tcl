# run_impl_bitstream_rowburst.tcl -- ZHR-92 (2026-09-14): real P&R for
# DWR_ROWBURST (per-row write request/response, lane-1 row buffer; CCOL
# achieved II=1) on top of the deployed dwob baseline. Same BD, route_design
# only. Judgment: WNS >= 0 (current +0.172), report the critical path (the
# three per-row loops + buffer restructure dwr_consume; the current worst
# paths are dwr_produce's carry chain and the route-dominated population).
set_param general.maxThreads 1

set part      "xc7z020clg400-1"
set proj_name "rowburstimp"
set script_dir [file normalize [file dirname [info script]]]
set proj_dir   "$script_dir/$proj_name"

set ip_mac_array "$script_dir/../fastvit_ip_v2/rowburst_export/s1/impl/ip"
...
open_run impl_1
set rpt_file "$proj_dir/utilization_rowburst.rpt"
