# package_add_only_ip.tcl -- Package the add-only Tier B minimal
...
# 200MHz P&R timing-closure test. Isolates whether add_worker alone
...
#
# 用法: vivado -mode batch -source package_add_only_ip.tcl -nolog -nojournal

set_param general.maxThreads 1

set script_dir [file normalize [file dirname [info script]]]
set rtl_dir    "$script_dir/../fastvit_ip_w8a4/tier_b_single"
set proj_name  "add_only_pack_proj"
set proj_dir   "$script_dir/$proj_name"
set ip_dir     [file normalize "$script_dir/../fastvit_ip_w8a4/tier_b_single_ip/add"]
...
set_property top add_only_top [current_fileset]
...
set_property name        add_only_top   $core
set_property display_name "FastVIT add Only (Tier B isolation test)" $core
set_property description "Standalone add_worker + private AXI4 adapters, isolated from all other workers, for a minimal-harness 200MHz timing test" $core
...
puts ""
puts "======================================================================"
puts " BUS INTERFACES DETECTED (add_only_top)"
puts "======================================================================"
foreach bif [ipx::get_bus_interfaces -of_objects $core] {
