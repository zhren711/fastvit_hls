# run_impl_bitstream_sma.tcl -- ZHR-92 (2026-09-12): real P&R for
# SHARED_MUL_ARMS (all seven remaining call sites removed from the shared
# 32x32 multiplier: PW_FLAT writeout row product -> rt/wr_row accumulators;
# ROW_READ oh*W -> accumulators; four per-chunk pw_ot_count products and the
# top-level scalar_hw/total -> narrow-typed multiplies on their own small
# cores; pw_ot_lo -> accumulator). Binding DB: zero 32x32 multiply ops in
# run_layer and at the top; mul_32s_32s_32_2_1 exists only inside DW's own
# dwr_consume3 (2, unchanged). csim 5/5+4/4+8/8+4/4. Isolated LUT -283, DSP
# -10, II unchanged. Same BD, route_design only. Judgment: (2) the shared
# unit absent from the top-10; (4) WNS vs +0.138 -- ~+0.25 expected (next-
# worst path at this placement), ~+0.18 means the store-unit path bites first.
set_param general.maxThreads 1

set part      "xc7z020clg400-1"
set proj_name "smaimp"
set script_dir [file normalize [file dirname [info script]]]
set proj_dir   "$script_dir/$proj_name"

set ip_mac_array "$script_dir/../fastvit_ip_v2/sma_export/s1/impl/ip"
...
open_run impl_1
set rpt_file "$proj_dir/utilization_sma.rpt"
