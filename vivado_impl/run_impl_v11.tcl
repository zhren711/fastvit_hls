# run_impl_v11.tcl — Upgrade dwconv_ip to v11.0 (32-bit packed), re-implement
#
# 步骤:
# 1. 打开 fastvit_util_check 项目
# 2. 更新 IP repo 路径 (dwconv_ip v11.0)
# 3. 检测并升级 BD 中的 IP (dwconv_ip 从 v10.0 → v11.0)
# 4. 重新生成 BD wrapper
# 5. 重跑 Synthesis + Implementation + Bitstream
#
# 用法: vivado -mode batch -source run_impl_v11.tcl -nolog -nojournal
# 或在 Vivado Tcl Console: source E:/codes/microzed/fastvit_hls/vivado_impl/run_impl_v11.tcl
...
puts ">>> Rebuilding IP catalog (picks up dwconv_ip v11.0)..."
update_ip_catalog -rebuild

# ── 升级 BD 中 dwconv_ip v10.0 → v11.0 ──────────────────
...
if {[llength $upgrade_ips] > 0} {
    ...
    puts ">>> No IPs need upgrading (check that v11.0 export is done)."
}
...
# ── 利用率 + 时序报告 ────────────────────────────────────
open_run impl_1
set rpt_file "$script_dir/fastvit_util_check/utilization_v11.rpt"
report_utilization -file $rpt_file
puts ""
puts "========================================"
puts " POST-IMPLEMENTATION UTILIZATION (v11) "
puts "========================================"
report_utilization
