# run_impl_mac_array_a3_dwraster_step2_lut.tcl -- ZHR-92, 2026-08-28, DW
# raster integration (Step 2), LUT-mode binding retry after the
# forced-DSP round routed but failed timing (WNS=-0.108ns, real P&R:
# 41,222 LUT/77.48%, 92 DSP/41.82%, critical path confirmed still in
# pw_flat_pipeline's mul_32s_32s_32_2_1 -- 4 logic levels, 41.5% route
# share, no new hot spot from the raster integration itself). That real
# P&R result invalidated the isolated-csynth net-increment estimates in
# BOTH directions (LUT -53%, DSP +38% off, opposite signs) -- see
# CLAUDE.md's new entry on this. So this LUT-mode run's own isolated
# csynth prediction (51 DSP/23.2%, 58.14% LUT) is NOT trusted either;
# this run exists to get the real number, not to confirm a predicted one.
#
# Same 4-master BD/SmartConnect topology, no pblock (this design's
# critical path showed 4 logic levels/41.5% route share last round, not
# the zero-logic-levels/high-route-share signature CLAUDE.md's pblock
# rule requires -- unless this round's real report shows something
# different, checked fresh, not assumed carried over).
#
# One shot; if this also fails to route or WNS goes negative, STOP and
# report -- do not chain into a second attempt blind. If LUT-mode also
# fails, that means the raster integration's real area growth itself
# exceeds this design's current timing margin, independent of DSP-vs-LUT
# binding choice.
#
# 用法: vivado -mode batch -source run_impl_mac_array_a3_dwraster_step2_lut.tcl -nolog -nojournal

set_param general.maxThreads 1

set part      "xc7z020clg400-1"
set proj_name "a3dwr2lut"
set script_dir [file normalize [file dirname [info script]]]
set proj_dir   "$script_dir/$proj_name"

set ip_mac_array "$script_dir/../fastvit_ip_v2/mp3/s1/impl/ip"
...
puts ""
puts "========================================"
puts " POST-ROUTE UTILIZATION (mac_array_top, PW_FLAT II=2->1 fix)"
puts "========================================"
report_utilization
