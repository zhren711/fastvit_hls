# run_impl_v14.tcl — Upgrade dwconv_ip to v14.0 (TR-sequential, fits xc7z020)
#
# 步骤:
# 1. 打开 fastvit_util_check 项目
# 2. 更新 IP repo 路径 (dwconv_ip v14.0 from dwconv_ip_proj_v14)
# 3. 检测并升级 BD 中的 IP (dwconv_ip v13.0 → v14.0)
# 4. 重新生成 BD wrapper
# 5. 重跑 Synthesis + Implementation + Bitstream
#
# 前提: 先运行 vitis_hls -f run_hls_v14.tcl (生成 v14.0 IP export)
# 用法: vivado -mode batch -source run_impl_v14.tcl -nolog -nojournal
...
# ── 更新 HLS IP repo 路径 (dwconv_ip → v14.0) ────────────
set ip_conv   "$script_dir/../conv_ip/conv_ip_proj/solution1/impl/ip"
set ip_dw     "$script_dir/../dwconv_ip/dwconv_ip_proj_v14/solution1/impl/ip"
...
puts ">>> Rebuilding IP catalog (picks up dwconv_ip v14.0)..."
update_ip_catalog -rebuild

# ── 升级 BD 中 dwconv_ip v13.0 → v14.0 ──────────────────
open_bd_design [get_files fastvit_bd.bd]

# First unlock any locked IPs (v13 may still be locked if catalog changed)
set locked_ips [get_bd_cells -filter {STATUS == "LOCKED"}]
if {[llength $locked_ips] > 0} {
    puts ">>> Found locked IPs: $locked_ips — attempting to fix..."
...
if {[llength $upgrade_ips] > 0} {
    ...
    puts ">>> No IPs need upgrading (already at v14.0 or check export)."
}
...
# ── 利用率 + 时序报告 ────────────────────────────────────
open_run impl_1
set rpt_file "$script_dir/fastvit_util_check/utilization_v14.rpt"
report_utilization -file $rpt_file
puts ""
puts "========================================"
puts " POST-IMPLEMENTATION UTILIZATION (v14) "
puts "========================================"
report_utilization
