# run_impl_dwconv_only_200mhz.tcl -- dwconv-only Tier B minimal harness:
# Block Design + synth + impl at 200MHz, to test whether dwconv_worker
...
# Companion to package_dwconv_only_ip.tcl (must run first). See
# typed-knitting-nygaard.md plan for context: HLS's own csynth estimate
# for dwconv_worker already misses 5ns in isolation (pre-P&R); this is
# the authoritative post-P&R check.
#
# 用法: vivado -mode batch -source run_impl_dwconv_only_200mhz.tcl -nolog -nojournal

set_param general.maxThreads 1

set part      "xc7z020clg400-1"
set proj_name "fastvit_dwconv_only_200mhz_proj"
set script_dir [file normalize [file dirname [info script]]]
set proj_dir   "$script_dir/$proj_name"

set ip_dir "$script_dir/../fastvit_ip_w8a4/tier_b_single_ip/dwconv"
...
create_bd_cell -type ip -vlnv user.org:hls:dwconv_only_top:1.0 worker_0
...
set idx 0
foreach m {gmem0 gmem1 gmem2 gmem3} {
    set si [format "S%02d_AXI" $idx]
    connect_bd_intf_net [get_bd_intf_pins worker_0/m_axi_dwconv_$m] [get_bd_intf_pins sc_main/$si]
    incr idx
}
...
# Round 5 (2026-08-07) TRIED AND REVERTED: a compact-floorplan Pblock
# (pblock_worker.xdc, bounding worker_0 to SLICE_X0Y0:SLICE_X113Y89 +
# matching DSP48/RAMB18 ranges, anchored to the Y0-59 DSP/BRAM column
# band) made timing WORSE (dwconv -0.196->-0.300ns, pwconv
# -0.480->-0.651ns) -- compacting the design increased routing
# congestion enough to outweigh the shorter average wire length. Not
# re-added here; see typed-knitting-nygaard.md for the full writeup.
...
# Round 3 (2026-08-07): both remaining critical paths are ~83% route
# delay / only 3-4 logic levels -- a placement-distance problem, not a
# logic-depth problem, so HLS/C++ source rework has hit diminishing
# returns. Post-ROUTE phys_opt_design (sees actual routed delays, unlike
# the post-PLACE pass above) is the standard lever for exactly this
# situation; ExtraTimingOpt trades placer runtime for a more
# timing-focused placement pass.
...
open_run impl_1
set rpt_file "$proj_dir/utilization_dwconv_only_200mhz.rpt"
report_utilization -file $rpt_file

set timing_rpt "$proj_dir/timing_detail_dwconv_only_200mhz.rpt"
report_timing -delay_type max -max_paths 10 -sort_by group -file $timing_rpt
puts ">>> Detailed timing report: $timing_rpt"
puts ""
puts "========================================"
puts " POST-IMPLEMENTATION UTILIZATION (dwconv-only, 200MHz)"
puts "========================================"
report_utilization
