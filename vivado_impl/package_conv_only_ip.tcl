# package_conv_only_ip.tcl -- Package the conv-only Tier B minimal
...
# 200MHz P&R timing-closure test. Isolates whether conv_worker alone
...
#
# 用法: vivado -mode batch -source package_conv_only_ip.tcl -nolog -nojournal

set_param general.maxThreads 1

set script_dir [file normalize [file dirname [info script]]]
set rtl_dir    "$script_dir/../fastvit_ip_w8a4/tier_b_single"
set proj_name  "conv_only_pack_proj"
set proj_dir   "$script_dir/$proj_name"
set ip_dir     [file normalize "$script_dir/../fastvit_ip_w8a4/tier_b_single_ip/conv"]
...
set_property top conv_only_top [current_fileset]
...
set_property name        conv_only_top   $core
set_property display_name "FastVIT conv Only (Tier B isolation test)" $core
set_property description "Standalone conv_worker + private AXI4 adapters, isolated from all other workers, for a minimal-harness 200MHz timing test" $core
...
puts ""
puts "======================================================================"
puts " BUS INTERFACES DETECTED (conv_only_top)"
puts "======================================================================"
foreach bif [ipx::get_bus_interfaces -of_objects $core] {
