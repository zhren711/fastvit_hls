 ...
 * dead code on the verified/deployed build. See
 * mac_array_rowread_overlap.cpp's own header comment on this identical
 * block for the full close-out writeup (mechanism correct: DATAFLOW
 * genuinely applied, numeric overlap proof total=max(produce,consume),
 * deadlock warning empirically cleared at 2 tested scales via real RTL
 * cosim -- but closed anyway: real whole-IP P&R failed timing, WNS=
 * -0.073ns, AND the win was structurally capped at ~4% even had it
 * passed, since produce -- the side overlap can hide -- is only ~4% of
 * the produce+consume pair; the real 5.24x-6.02x external cost is almost
 * entirely in consume's own FIFO-ordered, blocking drain, which
 * hls::burst_maxi's read() cannot parallelize). Duplicated here verbatim
 * (not refactored into a shared header) so this file's own diff against
 * mac_array_raster_integrated.cpp stays a clean, additive, single-concern
 * change, same discipline as every other file in this line. */
...
/* ZHR-92 line-C rescope, Step 3 (2026-08-29) -- see mac_array_rowread_overlap.cpp's
 * own identical comment for the full rationale (duplicated verbatim here,
 * same discipline as row_read_produce/consume above). Step 2 found
 * DATAFLOW silently dropped (HLS 207-5575) inside an `if` block; fix is
 * to extract the guarded block into its own function with DATAFLOW as
 * its first statement, matching dwr_produce9/dwr_consume10's own shape. */
