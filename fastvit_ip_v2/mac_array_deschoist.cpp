...
/* ZHR-92 descriptor-hoist, TERMINAL STATUS (2026-08-29). Mechanism
 * confirmed correct and real; line closed anyway -- measured payoff
 * didn't justify further work. Read this before reopening.
 *
 * WHAT THIS DOES: Cin/n_ot/out_shift/w_off/w_out/out_off/use_shift_table/
 * out_ch_stride are plain scalar parameters here, not fields read from a
 * LayerDescV2 reference -- this function is called once per (rt,colt)
 * OUTPUT TILE (up to 256+ times for real PW layers), and run_layer (the
 * caller) already computes every one of these values as its own local
 * scalar before the tile loop even starts. Reading them AGAIN here via
 * `d.field` on every tile call was a genuine, real gmem_meta (AXI-Lite
 * descriptor) re-fetch of information the caller already held --
 * confirmed via HLS's own burst-inference log. This is NOT option D
 * (2026-08-22, ATTEMPTED AND REVERTED, see mac_array_top's own comment on
 * it): that attempt copied the WHOLE LayerDescV2 struct by value once per
 * layer (synthesized real burst-FIFO/decode logic, +25% over LUT
 * capacity) on a theory ("gmem_meta's critical path is caused by high
 * fan-out") that its own P&R record already contradicted. This change
 * doesn't touch descriptor storage/access SHAPE at all -- it stops
 * re-deriving values from `d` the caller already has, by threading them
 * down as ordinary scalar arguments. `run_layer`'s own `pw_narrow`
 * computation had the identical bug (re-reading `d.out_off`/
 * `d.last_col_tile` every tile) and got the same fix. `dw_raster_layer.cpp`
 * checked and confirmed clean -- never referenced LayerDescV2 fields at
 * all, already scalar-passing.
 *
 * WHAT WAS MEASURED, real numbers, not assumptions:
 *   - csynth: gmem_meta burst-read sites 11->9, confirmed by exact line
 *     number, not just count -- zero burst reads trace into this
 *     function's body anymore.
 *   - Real whole-IP P&R (one shot): FAILED TIMING, WNS +0.021ns->
 *     -0.305ns. Route succeeded; this is a timing-only failure. Real
 *     resources: LUT -520/-1.26% (isolated csynth had predicted -2.7%,
 *     same direction, ~2x off magnitude -- 5th confirmed instance of
 *     "isolated csynth != whole-IP", see CLAUDE.md), DSP/BRAM unchanged.
 *   - THE critical path MOVED for the first time across an entire
 *     exploration line (baseline/111MHz/125MHz/X0-108-pblock had all
 *     shown the identical gmem_meta AXI-FIFO path, ~10ns/10 logic
 *     levels/80-83% route). Removing these 2 read sites relieved that
 *     path -- but a DIFFERENT, pre-existing near-critical path (this
 *     function's own FSM -> grp_fu_873, the SAME shared multiplier this
 *     project already traced to run_gelu/sigmoid/relu/add in an
 *     unrelated round) surfaced worse (8.786ns, 5 logic levels, 60.1%
 *     logic-dominated). CONFIRMED (not "suspected", see CLAUDE.md)
 *     instance of "reducing a call site's evaluation count doesn't
 *     reduce competitor count for a shared multiplier -- it can add
 *     new ones."
 *   - Board test (WNS-negative bitstream, latency-measurement only,
 *     never a deliverable, golden image untouched, restored after):
 *     two full-network runs, 3,608.56ms and 3,601.35ms, all 7
 *     checkpoints byte-exact identical between runs (no WNS-violation
 *     divergence). Real result: total time is UNCHANGED from the
 *     deployed baseline's 3,608.76ms (0.20ms/~0.006% difference, inside
 *     noise) -- the predicted latency win from eliminating per-tile
 *     redundant reads did NOT show up detectably end-to-end, despite up
 *     to 256 tiles/layer.
 *
 * DECISION: closed. Latency win not significant -> not worth chasing the
 * newly-surfaced shared-multiplier competition to recover timing. Kept
 * in the repo as a documented, mechanism-correct-but-no-measured-benefit
 * result -- do not re-derive this from scratch; the negative result is
 * the finding. Full trail: ZHR-92/ZHR-63, 2026-08-29 comments. */
template<bool FAST_WRITEOUT>
static void pw_flat_pipeline_impl(
    int Cin, int n_ot, int out_shift_layer, int w_off, int w_out, int out_off,
    int use_shift_table, int out_ch_stride,
    const wt_t w_base[],
    ...
    int rt, int colt, int r_sz, int col_sz)
{
    ...
    int shift_reg = out_shift_layer; /* recomputed at each ot's compute->writeout transition */
    ...
    PW_FLAT: for (int i = 0; i < total_iters; i++) {
        ...
        if (!in_writeout) {
            ...
            for (int dd = 0; dd < MAC_PD; dd++) {
                ...
                lane_w[dd] = ch_valid ? w_base[w_off + w_ot_base + ch_off + dd] : (wt_t)0;
                for (int rr = 0; rr < MAC_PR; rr++) {
                    #pragma HLS UNROLL
                    for (int cw = 0; cw < MAC_PC; cw++) {
                        #pragma HLS UNROLL
            ...
            if (wr_row < r_sz) {
                int byte_addr = out_off + ot_out_ch_base + (rt * MAC_PR + wr_row) * w_out + colt * MAC_PC;
                if (FAST_WRITEOUT) {
        ...
        if (k == wrap_bound - 1) {
            ...
            if (!in_writeout) {
                if (cbase_idx == n_cbase - 1) {
                    in_writeout = true;
                    shift_reg = use_shift_table ? (int)pw_shift_cache[ot_idx] : out_shift_layer;
                } else {
...
static void pw_flat_pipeline(
    ...
    int rt, int colt, int r_sz, int col_sz)
{
    pw_flat_pipeline_impl<true>(Cin, n_ot, out_shift_layer, w_off, w_out, out_off, use_shift_table, out_ch_stride,
        w_base, pw_patch_full, pw_bias_cache, pw_shift_cache, out_base, out_burst, rt, colt, r_sz, col_sz);
}
...
static void pw_flat_pipeline_narrow(
    ...
    int rt, int colt, int r_sz, int col_sz)
{
    pw_flat_pipeline_impl<false>(Cin, n_ot, out_shift_layer, w_off, w_out, out_off, use_shift_table, out_ch_stride,
        w_base, pw_patch_full, pw_bias_cache, pw_shift_cache, out_base, out_burst, rt, colt, r_sz, col_sz);
}
                       ...
                       hls::burst_maxi<ap_uint<32> > &in_burst)
{
    ...
    /* ZHR-92 line-A rescope (2026-08-29): PW-specific fields hoisted here,
     * once per layer, matching the discipline already used for Cin/n_ot
     * two lines up -- pw_flat_pipeline_impl (called once per (rt,colt)
     * tile, up to 256+ times/layer) used to read these directly off `d`
     * itself, re-fetching from gmem_meta every tile despite run_layer
     * already having the answer. Harmless to compute for DW layers too
     * (DW's own dispatch never reads these locals, same "unused for the
     * other op_type, zero-cost" convention as Cin/n_ot above). */
    const int pw_out_shift = d.out_shift;
    const int pw_w_off = d.w_off;
    const int pw_w_out = d.w_out;
    const int pw_out_off = d.out_off;
    const int pw_use_shift_table = d.use_shift_table;
    const int pw_out_ch_stride = d.out_ch_stride;
    const int pw_last_col_tile = d.last_col_tile;
    ...
    for (int rt = 0; rt < d.n_row_tiles; rt++) {
        ...
        for (int colt = 0; colt < d.n_col_tiles; colt++) {
            ...
            for (int ot = 0; ot < n_ot; ot++) {
                ...
                bool pw_narrow = (pw_last_col_tile < MAC_PC) || ((pw_out_off & 3) != 0);
                if (pw_narrow) {
                    pw_flat_pipeline_narrow(Cin, n_ot, pw_out_shift, pw_w_off, pw_w_out, pw_out_off, pw_use_shift_table, pw_out_ch_stride,
                        w_base, pw_patch_full, pw_bias_cache, pw_shift_cache, out_base, out_burst, rt, colt, r_sz, col_sz);
                } else {
                    pw_flat_pipeline(Cin, n_ot, pw_out_shift, pw_w_off, pw_w_out, pw_out_off, pw_use_shift_table, pw_out_ch_stride,
                        w_base, pw_patch_full, pw_bias_cache, pw_shift_cache, out_base, out_burst, rt, colt, r_sz, col_sz);
                }
            }
        }
