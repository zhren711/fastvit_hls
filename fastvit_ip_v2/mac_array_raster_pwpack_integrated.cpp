#include "mac_array.h"
// mac_array_raster_pwpack_integrated.cpp -- ZHR-92 DSP-packing Step 2
// (2026-08-29). Copy of mac_array_raster_integrated.cpp (the DW-raster
// milestone, kept pristine -- not touched by this file) with ONE
// additional change: the PW dispatch call site optionally routes to
// pw_flat_pipeline_packed() under #ifdef PW_ENABLE_PACKING (default
    ...
    for (int rt = 0; rt < d.n_row_tiles; rt++) {
        ...
        for (int colt = 0; colt < d.n_col_tiles; colt++) {
            ...
            for (int ot = 0; ot < n_ot; ot++) {
                ...
                /* ZHR-92 DSP-packing Step 2 (2026-08-29): scalar-only
                 * packed writeout this round (no FAST_WRITEOUT/out_burst
                 * variant yet -- see pw_pack_pipeline.h), so both the
                 * narrow and non-narrow real dispatch conditions route
                 * here alike; the fast/slow distinction pw_narrow encodes
                 * doesn't have a packed counterpart to select between yet.
                 * n_ot(=d.cout) is caller-guaranteed even (confirmed
                 * across all 12 distinct real PW cout values). */
                pw_flat_pipeline_packed(d, w_base, pw_patch_full, pw_bias_cache, pw_shift_cache, out_base, rt, colt, r_sz, col_sz);
#else
                if (pw_narrow) {
