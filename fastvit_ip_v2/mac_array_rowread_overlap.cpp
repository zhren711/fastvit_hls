...
#ifdef ROW_READ_OVERLAP
/* ZHR-92 line-C rescope, TERMINAL STATUS (2026-08-29) -- default OFF,
 * this whole block is dead code on the verified/deployed build. Kept in
 * the repo, not deleted, as a documented, mechanism-correct-but-not-
 * worth-integrating result. Read this header before re-opening line C.
 *
 * MECHANISM: CORRECT AND REAL, not in question.
 *   - Structural diagnosis (Step 1-3): ROW_READ_CH carried no PIPELINE/
 *     DATAFLOW of its own, so read_request(ci+1) never issued until
 *     read_request(ci)'s full drain completed -- only 1 of the
 *     interface's 16 outstanding-request slots was ever used.
 *   - Step 3 fix (extracting the DATAFLOW region into its own function,
 *     row_read_dataflow -- matching dwr_produce9/dwr_consume10's own
 *     shape) got DATAFLOW to genuinely apply: confirmed via HLS's own
 *     transform log (2 process functions extracted), independent module
 *     reports, Pipeline Type=dataflow, and -- the decisive number --
 *     a determinate-trip-count fixed-shape probe showing
 *     row_read_dataflow's own total latency = EXACTLY max(produce,
 *     consume), not sum, at two tested scales (Cin=8: 1091=max(42,1091);
 *     Cin=64: 8707=max(266,8707)). Real, numeric proof of overlap, not
 *     structural inference.
 *   - The HLS 200-656 deadlock warning (ap_ctrl_none + auto-rewind under
 *     DATAFLOW) was empirically cleared via two real RTL cosimulations
 *     (Cin=8 and an 8x stress case, Cin=64, specifically targeting the
 *     documented failure mode -- produce issuing 256 read_request calls,
 *     far exceeding num_read_outstanding=16) -- both PASS, byte-exact,
 *     HLS's own AESL_deadlock_detector did not fire in either run.
 *     Empirically cleared at tested scales; the static warning itself
 *     remains present in the csynth log -- this is not a formal proof
 *     across every possible relative timing skew.
 *
 * WHY THIS LINE IS CLOSED ANYWAY -- two independent reasons:
 *   1. Real whole-IP P&R (Step 4, one-shot per this line's own stop-loss
 *      discipline): route_design completed (all nets routed, verified)
 *      but FAILED TIMING, WNS=-0.073ns (baseline was +0.021ns). Real
 *      resource delta: +172 LUT (+0.42%) / +3 DSP -- note this is yet
 *      another, 4th, confirmed instance of "isolated csynth can't
 *      predict whole-IP", and this time in the OPPOSITE direction from
 *      the DW-raster round's own instance (that one underestimated LUT
 *      by 53%; isolated csynth THIS round overestimated LUT by 7.2x,
 *      1244 predicted vs 172 real) -- see CLAUDE.md, the direction isn't
 *      even consistent, not just the magnitude. Critical path stayed in
 *      the same general net family as the prior round (gmem_meta's own
 *      AXI-Lite descriptor-read FIFO, NOT row_read_produce/consume --
 *      confirmed via direct grep, dw_raster_layer.cpp/DW's own path is
 *      untouched) but its own shape changed substantially (0->10 logic
 *      levels, 74%->80.6% route share) -- this design's placement is
 *      sensitive enough that even an unrelated-looking change can nudge
 *      an existing near-critical path across zero; the THIRD confirmed
 *      instance of this exact pattern on this line (DW-raster's own
 *      297-hoist fix cost -0.355ns on a resource-neutral change; the
 *      forced-DSP-vs-LUT-mode round flipped WNS sign on matching
 *      resources). -0.073ns would not survive the next structural change
 *      any more than the two prior instances did.
 *   2. THE REAL, DELIBERATELY-NOT-OBVIOUS FINDING, independent of the
 *      P&R failure: even had timing passed, the win was capped far below
 *      what it looked like. Overlap hides produce inside consume's own
 *      runtime -- but produce is only 42/266 cycles against consume's
 *      1091/8707 (both fixed-shape measurements above): produce is
 *      already only ~4% of the pair's total cost. "1 request in flight
 *      vs. 16 supported" reads like a 16x throughput opportunity, but
 *      issuing a read_request is cheap; the real cost -- the 5.24x-6.02x
 *      external/on-chip-compute gap this whole line exists to attack --
 *      is almost entirely in CONSUME's own drain (real AXI round-trip
 *      latency), not in produce's issue side. Overlap can only ever hide
 *      the cheap ~4% side; it cannot touch the ~83% (per the round that
 *      measured total/internal=5.24x-6.02x) that's actually expensive.
 *      To use the interface's real outstanding-request capacity for the
 *      big number, CONSUME itself would need to drain multiple in-flight
 *      requests concurrently -- but hls::burst_maxi's read() is a
 *      strictly FIFO-ordered, blocking drain (verified directly against
 *      Xilinx's own hls_burst_maxi.h, not assumed): one process, one
 *      ordered stream of reads, no concurrent multi-request drain
 *      possible through this API. That is a real, structural blocker --
 *      not a placement or synchronization problem this line's own P&R
 *      difficulty could ever have been tuned around. */
static void row_read_produce(
    hls::burst_maxi<ap_uint<32> > &in_burst,
    int in_off, int in_ch_stride, int W, int Cin, int r_sz, int rt)
{
    for (int rr = 0; rr < MAC_PR; rr++) {
        bool r_valid = rr < r_sz;
        int oh = rt * MAC_PR + (r_valid ? rr : 0);
        int ch_base = 0;
        ROW_READ_PRODUCE_CH: for (int ci = 0; ci < Cin; ci++) {
            #pragma HLS PIPELINE II=1
            int byte_addr = in_off + ch_base + oh * W;
            int word_addr0 = byte_addr >> 2;
            int r = byte_addr & 3;
            int n_words = (r + W + 3) >> 2;
            in_burst.read_request((size_t)word_addr0, (unsigned)n_words);
            ch_base += in_ch_stride;
        }
    }
}

static void row_read_consume(
    hls::burst_maxi<ap_uint<32> > &in_burst,
    int in_off, int in_ch_stride, int W, int Cin, int r_sz, int rt,
    act_t row_buf[MAC_PR][MAX_CIN_TIMES_W])
{
    for (int rr = 0; rr < MAC_PR; rr++) {
        ...
        int ch_base = 0;
        int flat_base = 0;
        ROW_READ_CONSUME_CH: for (int ci = 0; ci < Cin; ci++) {
            int byte_addr = in_off + ch_base + oh * W;
            int r = byte_addr & 3;
            int n_words = (r + W + 3) >> 2;
            ROW_READ_CONSUME_FILL: for (int i = 0; i < MAX_WORDS_PER_CH; i++) {
                #pragma HLS PIPELINE II=1
...
/* ZHR-92 line-C rescope, Step 3 (2026-08-29): Step 2 found #pragma HLS
 * DATAFLOW silently dropped (HLS 207-5575) when placed as the first
 * statement inside an `if` block -- HLS only accepts it as the first
 * statement of a `for` loop body or a function body. Fix: extract the
 * whole row_hoist_ok-guarded block into its own function, DATAFLOW as
 * that function's own first statement, `if` left at the call site --
 * same shape as dw_raster_layer.cpp's own dwr_produce9/dwr_consume10,
 * which already appear as independent module instances in csynth's own
 * module hierarchy (confirmed, not assumed, in that file's own history). */
static void row_read_dataflow(
    hls::burst_maxi<ap_uint<32> > &in_burst,
    int in_off, int in_ch_stride, int W, int Cin, int r_sz, int rt,
    act_t row_buf[MAC_PR][MAX_CIN_TIMES_W])
{
    #pragma HLS DATAFLOW
    row_read_produce(in_burst, in_off, in_ch_stride, W, Cin, r_sz, rt);
    row_read_consume(in_burst, in_off, in_ch_stride, W, Cin, r_sz, rt, row_buf);
}
#endif // ROW_READ_OVERLAP
                       ...
                       hls::burst_maxi<ap_uint<32> > &in_burst)
{
    ...
    for (int rt = 0; rt < d.n_row_tiles; rt++) {
        ...
        if (row_hoist_ok) {
#ifdef ROW_READ_OVERLAP
            row_read_dataflow(in_burst, d.in_off, d.in_ch_stride, W, Cin, r_sz, rt, row_buf);
#else
