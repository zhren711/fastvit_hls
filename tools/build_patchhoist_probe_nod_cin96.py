"""
build_patchhoist_probe_nod_cin96.py -- A3 angle-B, n_cbase decomposition
probe (ZHR-92, 2026-08-24). Third point isolating whether the 632
cycles/ot gap found by the cout=96 probe is dominated by pw_flat_pipeline's
compute steps (n_cbase*16 per ot) or its WRITEOUT steps (16 per ot,
unchanged by n_cbase): cout=48 held fixed (so n_ot and WRITEOUT's own
16-elements/ot are UNCHANGED from the baseline), cin doubled 48->96
(n_cbase 2->3), spatial size h_in=w_in=64 (256 tiles, same as baseline).

PW_PATCH_HOIST's own cost DOES change here (it's cin-proportional, h=
3.455us/channel measured previously) -- that's expected and gets
subtracted out using the known h, isolating pw_flat_pipeline's own
per-ot-per-cbase slope.
"""
import struct
import os

ROOT = r"E:\codes\microzed\fastvit_hls"
OUT_DIR = os.path.join(ROOT, "accuracy_test_imgs_256", "board_test_patchhoist_nod_cin96")
...
in_buf = bytes([1]) * (cin * h_in * w_in)           # 393,216 bytes, all 1
...
assert n_row_tiles == 16 and n_col_tiles == 16 and n_cbase == 3 and cout == 48
print(f">>> total tiles = {n_row_tiles * n_col_tiles} (expect 256), n_cbase={n_cbase} (expect 3, baseline was 2)")

ref_out = bytes([96]) * (cout * h_out * w_out)  # sum_{ci=0..95}(1*1)=96, hand-computed
...
print(f">>> ref_out.bin: {len(ref_out)} bytes (every byte = 96, hand-computed)")
