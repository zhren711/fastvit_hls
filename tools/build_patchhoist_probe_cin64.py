"""
build_patchhoist_probe_cin64.py -- A3 PW_PATCH_HOIST cost-vs-Cin probe,
cin=64 point (ZHR-92, 2026-08-24). See build_patchhoist_probe_cin32.py's
header comment for the full method -- same shape (cout=48, h_in=w_in=32,
64 tiles) at cin=64 instead of 32, giving n_cbase=2 instead of 1 (one
extra 16-step cbase pass through the flat pipeline, a known, small,
precisely computable +23.04us delta at the flat-pipeline level -- NOT
part of what PW_PATCH_HOIST itself costs).
"""
import struct
import os

ROOT = r"E:\codes\microzed\fastvit_hls"
OUT_DIR = os.path.join(ROOT, "accuracy_test_imgs_256", "board_test_patchhoist_cin64")
...
in_buf = bytes([1]) * (cin * h_in * w_in)          # 65,536 bytes, all 1
w_buf = bytes([1]) * (cout * cin * k * k)           # 3,072 bytes, all 1
...
ref_out = bytes([64]) * (cout * h_out * w_out)  # sum_{ci=0..63}(1*1)=64, hand-computed
...
print(f">>> ref_out.bin: {len(ref_out)} bytes (every byte = 64, hand-computed)")
