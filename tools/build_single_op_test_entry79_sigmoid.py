"""
build_single_op_test_entry79_sigmoid.py -- A3 board bring-up (ZHR-92,
2026-08-23): package entry[79] (SIGMOID, real SE-block shape, the ONLY
SIGMOID in the whole 82-entry network) into a compact single-op board
test bundle. SIGMOID has never been board-tested on this architecture --
csim-only until now (mac_array_tb.cpp Phase 6, as part of the whole SE
flow).

Real input is entry[78]'s (fc2) real output (g_hw_seq[78].out_off=786432
== g_hw_seq[79].in_off), reference is entry_79.bin.
"""
import struct
import os

ROOT = r"E:\codes\microzed\fastvit_hls"
OUT_DIR = os.path.join(ROOT, "accuracy_test_imgs_256", "board_test_entry79_sigmoid")
...
with open(os.path.join(ROOT, "accuracy_test_imgs_256", "entry_78.bin"), "rb") as f:
    in_buf = f.read()
assert len(in_buf) == cin * h_in * w_in, f"entry_78.bin size {len(in_buf)} != {cin*h_in*w_in}"
