"""
build_single_op_test_entry60.py -- ZHR-92 (2026-09-03): package entry[60]
(layer_0040_pwconv, cin=384/cout=384, w_bytes=147,456 == PW_WEIGHT_CACHE_ELEMS
EXACTLY) into a single-op board test bundle, mirroring
build_single_op_test_entry64.py's own structure exactly.

This is the degenerate-path (pw_n_chunks==1) regression case for the
PW_WCHUNK chunked-loading board round -- the exact 144KB cache boundary
layer, previously the largest layer served by the OLD single-load cutoff.
Must be bit-identical to the pre-chunking build's own behavior.

entry[60]'s real input is entry[59]'s real output; entry_60.bin is the
byte-exact reference (both freshly dumped via mac_array_ckpt_dump.cpp,
extended to cover i>=59 this round).
"""
import struct
import os

ROOT = r"E:\codes\microzed\fastvit_hls"
OUT_DIR = os.path.join(ROOT, "accuracy_test_imgs_256", "board_test_entry60")
...
real_w_off, real_b_off, real_shift_off = 1119888, 7248, 3021792

W_SLICE_BYTES = cout * cin * k * k          # 147456
SHIFT_SLICE_BYTES = cout                     # 384
B_SLICE_ELEMS = cout                         # 384
...
with open(os.path.join(ROOT, "accuracy_test_imgs_256", "entry_59.bin"), "rb") as f:
    in_buf = f.read()
assert len(in_buf) == cin * h_in * w_in, f"entry_59.bin size {len(in_buf)} != {cin*h_in*w_in}"

with open(os.path.join(ROOT, "accuracy_test_imgs_256", "entry_60.bin"), "rb") as f:
    ref_out = f.read()
