"""
build_single_op_test_entry7.py -- ZHR-92 (2026-08-24): isolated board test
for entry[7] (cin=48, cout=144, 64x64 PWCONV) -- the first real-network
layer at 3x entry3's n_ot (144 vs 48), stress-testing hls::burst_maxi's
own repeat-invocation count (576 write_request/write/write_response
cycles vs entry3's 192) for the first time on real hardware. Isolates
this ONE variable (n_ot/burst-call-count) before considering entry9's
different variable (cin/n_cbase scale).

Real fields (w_off/b_off/shift_off/use_shift_table) pulled directly from
desc_all.bin's entry7, not hand-derived. entry_06.bin/entry_07.bin are
the real csim-dumped chain's input/reference for this exact layer
(entry7's input = entry6's real output).
"""
import struct
import os

ROOT = r"E:\codes\microzed\fastvit_hls"
OUT_DIR = os.path.join(ROOT, "accuracy_test_imgs_256", "board_test_entry7")
...
real_w_off, real_b_off, real_shift_off = 5520, 192, 3014736  # from desc_all.bin entry7

W_SLICE_BYTES = cout * cin * k * k          # 6912
SHIFT_SLICE_BYTES = cout                     # 144
B_SLICE_ELEMS = cout                         # 144 int32 words
...
with open(os.path.join(ROOT, "accuracy_test_imgs_256", "entry_06.bin"), "rb") as f:
    in_buf = f.read()
assert len(in_buf) == cin * h_in * w_in, f"entry_06.bin size {len(in_buf)} != {cin*h_in*w_in}"

with open(os.path.join(ROOT, "accuracy_test_imgs_256", "entry_07.bin"), "rb") as f:
    ref_out = f.read()
...
print(f">>> burst_maxi invocations expected: {cout} ot * {MAC_PR} rows = {cout*MAC_PR} (3x entry3's 192)")
